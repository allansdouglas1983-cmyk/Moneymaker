...
def _ref(mid, event_id, mtype, mtime=1000, name="A v B"):
    return L.MarketRef(market_id=mid, event_id=event_id, market_type=mtype,
                       market_time_ms=mtime, event_name=name)


# --------------------------------------------------------------------- catalogue dedup
def test_duplicate_packaging_is_collapsed_not_double_counted():
...
# ---------------------------------------------------------------------- link by event
def test_links_derivatives_to_mo_by_event_id():
...
def test_linkage_is_by_event_id_not_display_name():
    cat = L.build_catalogue([
...
def test_multiple_match_odds_for_event_is_flagged():
    cat = L.build_catalogue([
...
def test_ambiguous_when_two_derivatives_share_role_and_event():
    cat = L.build_catalogue([
        _ref("1.mo", "E1", "MATCH_ODDS"),
...
def test_marketTime_mismatch_is_visible():
    cat = L.build_catalogue([
...
def test_derivative_without_committed_mo_event_is_not_linked():
    cat = L.build_catalogue([
...
def test_role_classification_uses_parser_roles():
    cat = L.build_catalogue([
        _ref("1.mo", "E1", "MATCH_ODDS"),
