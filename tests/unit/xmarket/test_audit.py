...
# ------------------------------------------------------------------------- fixtures
def _md(mid, pt, mtype, event="E1", status="OPEN", in_play=False, mtime=2000):
...
def _rc(mid, pt, sel, hc, *, bb=None, bl=None, tv=0.0):
    ch = {"id": sel, "hc": hc, "ltp": 0.0, "tv": tv}
    if bb is not None:
        ch["batb"] = [[0, bb[0], bb[1]]]
    if bl is not None:
        ch["batl"] = [[0, bl[0], bl[1]]]
...
def _two_sided_tg(mid, event="E1"):
    return [
...
def _two_sided_gh(mid, event="E1"):
    return [
...
def _audit_one(cohort="STRICT", tour="ATP", cutoff_anchor=1000):
    cat = _catalogue()
...
# ------------------------------------------------------------- data-contract (§16)
def test_record_has_no_outcome_field():
    rec = _audit_one()
...
def test_clean_pair_is_usable_within_cutoff():
    rec = _audit_one()
...
def test_quote_age_cutoff_excludes_stale_book():
...
def test_n_primary_identifying_by_cutoff_cohort_tour():
...
def test_funnel_counts_every_refusal_reason():
    cat = build_catalogue([
        MarketRef("1.mo", "E1", "MATCH_ODDS", 2000),
        MarketRef("1.tg", "E1", "COMBINED_TOTAL", 2000),
    ])
...
def test_determinism_same_inputs_same_records():
    a = _audit_one()
    b = _audit_one()
...
def test_independent_update_status_present_when_derivatives_self_quote():
    recs = _sample_records()
