...
class _UniformFamily:
    ...
    def fit(
        self, races: object, schema: object, *, horizon: object, max_iter: int
    ) -> _MetadataFreeModel:
        return _MetadataFreeModel()

    def predict(self, model: object, race: Race, *, horizon: object) -> Mapping[int, float]:
        active = [r.runner_id for r in race.runners if not r.non_runner]
        return {rid: 1.0 / len(active) for rid in active}


class _RefusingFamily:
    family_id = "test-refusing-v1"

    def fit(
        self, races: object, schema: object, *, horizon: object, max_iter: int
    ) -> _MetadataFreeModel:
        raise StageOneFitRefusal("this family cannot fit this corpus")
...
class TestOrchestratorIsModelIndependent:
    ...
    def test_family_fit_refusal_is_an_explicit_exclusion(self) -> None:
        result = cross_fit(_corpus(), SCHEMA, horizon=H, family=_RefusingFamily())
