...
Features for a fixture must be supplied per-fixture under a ``features`` key. That is
deliberately manual for now: computing them live requires walking the archive to today, and
a predictor that silently built its features by a different code path from the harness that
validated it would be an untested model wearing a tested one's confidence interval. An
absent or partial feature map prices the fixture at the market, which is the honest answer
